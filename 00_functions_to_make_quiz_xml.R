...
##----------------------------------##
## Name: xml_quiz_creater
## Purpose: makes data frame into a XML format
## Input: question_bank_df,title,time_limit,max_attempts
## Output: XML code
## Notes:
##----------------------------------##
xml_quiz_creater = function(question_bank_df,title,time_limit,max_attempts){
...
##----------------------------------##
## Name: export_quiz_xml
## Purpose: export quiz as XML
## Input: short title, long title, quiz data frame
## Output: XML file
## Notes:
##----------------------------------##
export_quiz_xml = function(quiz_df,quiz_title, short_quiz_title) {## Set time limit (minutes or 'unlimited') 
  time_limit = 'unlimited'   
  
  ## Set Max Attempts 
  max_attempts = '100'  
  
  quiz_xml = xml_quiz_creater(quiz_df,quiz_title,time_limit,max_attempts)
  
  file_name = paste0(quiz_title,'_', 'test.xml')
  #where you are writting the file to
  write(quiz_xml, file = paste0(quiz_title,'_', 'test.xml'))
  zip(paste0(short_quiz_title,'_','myZippedQuiz.zip'), file_name)
}
